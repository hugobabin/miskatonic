...
def read_csv(data_in, data_treated, data_log):
    ...
    for file_path in get_csv_files(data_in):
        ...
        if missing:
            rapport_etl("structure", f"Missing columns: {sorted(missing)}", data_log,file=file_name)
            move_file(file_path, data_treated)
            continue
        ...
        for i, choice_col in enumerate(["responsea", "responseb", "responsec", "responsed"]):
            ...
            if not answers:
                if i in correct_indices:
                    rapport_etl("CORRECT_MISSING_CHOICE",
                            f"line={int(row['source_idx'])} col={choice_col}, file='{row['source_file']}'",
                            file=row["source_file"], line=int(row["source_idx"]))
                continue
            records.append({
    ...
    for _, row in df.sort_values("order").iterrows():
        ...
        if response_text in seen:
            if is_correct and not seen[response_text]["isCorrect"]:
                ...
                rapport_etl("ANSWER_MERGED", f"answer='{response_text}'",file=file_,line=line_)
            else:
                rapport_etl("ANSWER_IGNORED", f"answer='{response_text}'",file=file_,line=line_)
        else:
...
def build_question_object(question, subj, question_df, src_name, author, use):
    ...
    if errors:
        ...
        rapport_etl("QUESTION_REJECTED", f"{question} -> {','.join(errors)}",file=src_name,line=line_hint)
        return None
...
def export_questions_to_mongo(src_name, responses_df, author=None):
    ...
    for (qkey, subj, use), question_df in responses_df.groupby(["question_key","subject","use"], sort=False):
        ...
        rapport_etl("QUESTION_INSERTED",
                ...
                file=src_name, line=int(question_df["source_idx"].min()))
...
if __name__ == "__main__":
    ...
    if not files:
        ...
        print(f"Questions accepted: {stats['accepted']} | rejected: {stats['rejected']} | total: {stats['total']}")
